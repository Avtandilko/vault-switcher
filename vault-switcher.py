...
def duplicate_path_with_new_name(client, mount_point, source_path, dest_path):
    read_response = client.secrets.kv.read_secret_version(mount_point=mount_point, path=source_path)
    ...
    log.info("Path %s/%s duplicated with the name %s/%s", mount_point, source_path, mount_point, dest_path)
...
if __name__ == "__main__":
    ...
    logging.basicConfig(stream=sys.stdout, level=logging.INFO, format='{"timestamp": "%(asctime)s", "level": "%(levelname)s", "message": "%(message)s"}')
    ...
    if check_if_path_exists(client, VAULT_MOUNT_POINT, VAULT_SOURCE_PATH):
        if check_if_path_exists(client, VAULT_MOUNT_POINT, VAULT_DEST_PATH):
            ...
            duplicate_path_with_new_name(client, VAULT_MOUNT_POINT, VAULT_SOURCE_PATH, VAULT_DEST_PATH)
    else:
